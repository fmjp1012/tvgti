...
def main():
    ...
    # オフライン最適（各tで f(s) を最小化したと仮定した平均フロベニウス誤差）
    def compute_offline_mean_error(X_mat: np.ndarray, S_series_true, gamma_val: float) -> float:
        N_loc = X_mat.shape[0]
        T_loc = X_mat.shape[1]
        ...
        for t in range(T_loc):
            ...
            Q = Q_t.toarray() if hasattr(Q_t, 'toarray') else Q_t
            rhs = 2.0 * sigma_t
            try:
                s_star = np.linalg.solve(Q + 1e-8 * np.eye(Q.shape[0]), rhs)
            except np.linalg.LinAlgError:
                s_star = np.linalg.lstsq(Q + 1e-6 * np.eye(Q.shape[0]), rhs, rcond=None)[0]
            ...
            errors.append(np.linalg.norm(S_mat - S_series_true[t], ord='fro'))
    ...
    # オフライン平均誤差を横線（点線）で追加
    pc_offline_err = compute_offline_mean_error(X, S_series, gamma)
    co_offline_err = compute_offline_mean_error(X, S_series, gamma)
    sgd_offline_err = compute_offline_mean_error(X, S_series, 0.0)
