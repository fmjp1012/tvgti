...
def main():
    ...
    parser.add_argument("--show_offline_line", type=int, default=1, help="オフラインNSEの横線を描画する(1)/しない(0)")
    ...
    # オフライン最適（SEM+L1）の解を一括で求める
    def compute_offline_solution_l1(X_mat: np.ndarray, lambda_l1: float) -> np.ndarray:
    ...
    S_offline = compute_offline_solution_l1(X, lambda_reg)
    denom = (np.linalg.norm(S_offline) ** 2) + 1e-12

    # 誤差をNSEに変更
    err_pp = [np.linalg.norm(S_hat_list[t] - S_offline) ** 2 / denom for t in range(T)]
    err_pc = [np.linalg.norm(estimates_pc[t] - S_offline) ** 2 / denom for t in range(T)]
    err_co = [np.linalg.norm(estimates_co[t] - S_offline) ** 2 / denom for t in range(T)]
    err_sgd = [np.linalg.norm(estimates_sgd[t] - S_offline) ** 2 / denom for t in range(T)]
    err_pc_l1c = [np.linalg.norm(estimates_pc_l1c[t] - S_offline) ** 2 / denom for t in range(T)]
    ...
    # オフライン平均NSE（横線、点線）はフラグで制御
    if args.show_offline_line:
        offline_nse = float(np.mean([np.linalg.norm(S_offline - S_series[t]) ** 2 / denom for t in range(T)]))
        plt.axhline(y=offline_nse, color='black', linestyle='--', alpha=0.8, label='Offline SEM+L1 (mean NSE)')
    plt.legend()
