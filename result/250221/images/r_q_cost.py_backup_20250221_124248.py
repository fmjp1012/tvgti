...
def calc_cost_series(estimates_list: List[np.ndarray], X: np.ndarray) -> List[float]:
    cost_arr = []
    for S_est in tqdm(estimates_list, desc="Calculating cost series", leave=False):
        cost_val = np.linalg.norm(X - S_est @ X)**2
...
#----------------------------------------------------
# r を変化させた Proposed 手法のコスト関数の推移を計算
pp_cost_for_r = {}  # {r_val: [cost(t=0), cost(t=1), ...], ...}
if run_pp_r_flag:
    for r_val in r_list:
        estimates_r = pp_estimates_for_r[r_val]
        pp_cost_for_r[r_val] = calc_cost_series(estimates_r, X)
