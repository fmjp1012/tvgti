...
#----------------------------------------------------
# sgd手法実行関数定義
def run_tv_sem_sgd(beta_sgd: float, lambda_sgd: float) -> List[np.ndarray]:
    """Run the sgd method with specified hyperparams."""
    tv_sem_sgd = TimeVaryingSEM_PC_SPARSE(
        N, S_0,  lambda_sgd, 0, beta_sgd, gamma, P, C, name="sgd"
    )
...
best_lambda_sgd = best_trial.params["lambda_sgd"]

print(f"Best Hyperparams => beta_sgd={best_beta_sgd}, lambda_sgd={best_lambda_sgd}, gamma={gamma}, P={P}, C={C}")

#----------------------------------------------------
# チューニング済みパラメータで最終実行（sgd手法）
#----------------------------------------------------

estimates_sgd_tuned = run_tv_sem_sgd(beta_sgd=best_beta_sgd, lambda_sgd=best_lambda_sgd)
...
filename: str = (
    ...
    f'lambda_sgd{best_lambda_sgd}_'
    f'gamma{gamma}_'
    f'P{P}_'
    f'C{C}.png'
)
