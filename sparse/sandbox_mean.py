...
beta_co = 0.02
beta_sgd = 0.0269
lambda_reg = 0.01  # regularization parameter for sparse models
...
def run_trial(trial_seed):
    ...
    if run_pc_flag:
        tv_sem_pc = TimeVaryingSEM_PC_SPARSE(N, S_0, lambda_reg, alpha, beta_pc, gamma, P, C, False)
    ...
    # Correction Only
    if run_co_flag:
        tv_sem_co = TimeVaryingSEM_PC_SPARSE(N, S_0, lambda_reg, alpha, beta_co, gamma, 0, C, False)
    ...
    # SGD
    if run_sgd_flag:
        tv_sem_sgd = TimeVaryingSEM_PC_SPARSE(N, S_0, lambda_reg, alpha, beta_sgd, 0, 0, C, False)
